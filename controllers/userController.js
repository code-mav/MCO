...
exports.update_reservation = async (req, res) => {
  try {
    const { id } = req.params;
    const { location, room, time, date } = req.body;
    const updatedReservation = { location, room, time, date };
    const reservation = await Reservation.findById(id).lean(); 
    ...
    console.log("✅ Reservation updated:", new_reservation);
    return res.json({success: "✅ Reservation updated successfully!"}) // Redirect to view reservations with updated date
  } catch (error) {
